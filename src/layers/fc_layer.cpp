      ...
      input_cache(1, 1, input_size)
{
    ...
    // Initialize gradient buffers to zero to prevent garbage reads
    dW_ = Eigen::MatrixXf::Zero(output_size, input_size);
    dB_ = Eigen::VectorXf::Zero(output_size);
}

Tensor FCLayer::forward(const Tensor& tensor_input) 
{
    if (tensor_input.size() != input_size) {
        throw std::invalid_argument("Input size mismatch in FCLayer::forward");
    }
